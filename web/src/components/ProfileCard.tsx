...
export function ProfileCard({ profile }: { profile: Profile }) {
  const meetingFormat = profile.format === "either" ? "Online or in person" : profile.format === "online" ? "Online" : "In person";
  const headlineDetails = [profile.major, profile.location].filter(Boolean);
  const logistics = [meetingFormat, profile.availability.summary].filter(Boolean);
    ...
    <article className="group flex h-full flex-col rounded-[1.75rem] border border-cream-200 bg-white p-5 shadow-card transition hover:-translate-y-1 hover:border-coral-200 hover:shadow-card-hover">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0">
          ...
          {headlineDetails.length > 0 && <p className="mt-0.5 text-sm text-ink-500">{headlineDetails.join(" · ")}</p>}
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs font-medium text-ink-500">{logistics.join(" · ")}</div>
