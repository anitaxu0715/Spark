          ...
          return (
            <article className="rounded-[1.75rem] border border-cream-200 bg-white p-5 shadow-card sm:p-6" key={request.id}>
              ...
              <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2">
                <div><dt className="eyebrow">Suggested time</dt><dd className="mt-1.5 font-semibold text-indigo-950"><time dateTime={request.preferredTime} suppressHydrationWarning>{new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeStyle: "short" }).format(new Date(request.preferredTime))}</time></dd></div>
              ...
              {pendingProposal && (
                <section className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4">
                  ...
                  <p className="mt-2 text-sm text-ink-600">
                    {new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeStyle: "short" }).format(new Date(pendingProposal.proposedAt))}
